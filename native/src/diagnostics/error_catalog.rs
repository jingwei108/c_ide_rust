...
pub fn lookup_error_info(code: i32) -> Option<ErrorInfo> {
    Some(match code {
        ...
        1001 => ErrorInfo {
            ...
            emoji: "❓",
            title: "无法识别的字符",
            explanation: "程序中出现了 C 语言不支持的字符。请检查是否使用了中文标点（如中文分号；）或特殊符号。",
            common_causes: &["使用了中文标点符号", "复制粘贴时引入了不可见字符", "误按了特殊键"],
        },
        1002 => ErrorInfo {
            code: 1002,
            emoji: "💬",
        ...
        2001 => ErrorInfo {
            ...
            emoji: "📝",
            title: "预期类型名称",
            explanation: "这里需要一个类型名称（如 int、char、float 或自定义 struct 类型）。请检查是否拼写错误或遗漏了类型。",
            common_causes: &["类型名拼写错误", "忘记声明 struct 就使用"],
        },
        2002 => ErrorInfo {
            code: 2002,
            emoji: "📐",
        ...
        3001 => ErrorInfo {
            ...
            emoji: "🔄",
            title: "变量重复声明",
            explanation: "在同一个作用域内，同一个变量名只能声明一次。如果需要修改值，请直接赋值，不要重新声明。",
            common_causes: &["同一函数内多次 int x;", "for 循环变量与外层的变量同名"],
        },
        3002 => ErrorInfo {
            code: 3002,
            emoji: "🔄",
        ...
        3050 => ErrorInfo {
            ...
            emoji: "⚠️",
            title: "赋值 vs 比较",
            explanation: "条件中使用了赋值 =，可能是想使用比较 ==。if (a = b) 会把 b 的值赋给 a，然后判断 a 是否为 0。",
            common_causes: &["把 == 写成了 ="],
        },
        3051 => ErrorInfo {
            code: 3051,
