...
pub(crate) use crate::session::{AlgorithmMatch, Session};
pub(crate) use crate::unified::types::{AlgorithmStepSnapshot, ApiVariableSnapshot};
...
pub(crate) fn get_source_line(session: &Session, code_line: i32) -> Option<String> {
    let unit = session.compile.compile_units.first()?;
    let line = unit.source.lines().nth((code_line - 1) as usize)?;
    Some(line.trim().to_string())
}

pub(crate) fn find_algorithm_for_func<'a>(session: &'a Session, func_name: &str) -> Option<&'a AlgorithmMatch> {
...
impl<'a> VarMap<'a> {
    pub(crate) fn new(vars: &'a [ApiVariableSnapshot]) -> Self {
        Self { vars }
    }
