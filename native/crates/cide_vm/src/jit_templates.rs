...
use crate::core::{CideVM, StepResult, MEM_SIZE, NULL_TRAP_SIZE};
use crate::instruction::SourceLoc;
use crate::jit_trace::{JitTrace, MAX_TRACE_ITERATIONS};
use crate::opcode::OpCode;

pub type JitFn = fn(&mut CideVM, i32, i32, &SourceLoc, &mut VmContext<'_>) -> Option<StepResult>;
...
fn tpl_nop(_vm: &mut CideVM, _a: i32, _b: i32, _loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    None
}
...
fn tpl_push_const(
    ...
    _b: i32,
    _loc: &SourceLoc,
    _session: &mut VmContext<'_>,
) -> Option<StepResult> {
    vm.push(val as u64);
    None
}

fn tpl_pop(vm: &mut CideVM, _a: i32, _b: i32, _loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    vm.pop();
    None
}

fn tpl_dup(vm: &mut CideVM, _a: i32, _b: i32, loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
...
fn tpl_swap(vm: &mut CideVM, _a: i32, _b: i32, loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
...
fn tpl_load_global(
    ...
    let addr = crate::core::GLOBAL_START + offset as u32;
    if addr as u64 + 4 > MEM_SIZE as u64 || addr < NULL_TRAP_SIZE {
...
fn tpl_add(vm: &mut CideVM, _a: i32, _b: i32, _loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as i32;
    let a = vm.pop() as i32;
...
fn tpl_sub(vm: &mut CideVM, _a: i32, _b: i32, _loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as i32;
    let a = vm.pop() as i32;
...
fn tpl_mul(vm: &mut CideVM, _a: i32, _b: i32, _loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as i32;
    let a = vm.pop() as i32;
...
fn tpl_div(vm: &mut CideVM, _a: i32, _b: i32, loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as i32;
    let a = vm.pop() as i32;
    if b == 0 {
...
fn tpl_mod(vm: &mut CideVM, _a: i32, _b: i32, loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as i32;
    let a = vm.pop() as i32;
    if b == 0 {
...
fn tpl_uneg(vm: &mut CideVM, _a: i32, _b: i32, _loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let a = vm.pop() as u32;
...
fn tpl_eq(vm: &mut CideVM, _a: i32, _b: i32, _loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as i32;
    let a = vm.pop() as i32;
...
fn tpl_ne(vm: &mut CideVM, _a: i32, _b: i32, _loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as i32;
    let a = vm.pop() as i32;
    vm.push(if a != b { 1 } else { 0 });
    None
...
fn tpl_lt(vm: &mut CideVM, _a: i32, _b: i32, _loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as i32;
    let a = vm.pop() as i32;
    vm.push(if a < b { 1 } else { 0 });
    None
...
fn tpl_le(vm: &mut CideVM, _a: i32, _b: i32, _loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as i32;
    let a = vm.pop() as i32;
    vm.push(if a <= b { 1 } else { 0 });
    None
...
fn tpl_gt(vm: &mut CideVM, _a: i32, _b: i32, _loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as i32;
    let a = vm.pop() as i32;
    vm.push(if a > b { 1 } else { 0 });
    None
...
fn tpl_ge(vm: &mut CideVM, _a: i32, _b: i32, _loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as i32;
    let a = vm.pop() as i32;
    vm.push(if a >= b { 1 } else { 0 });
    None
...
fn tpl_and(vm: &mut CideVM, _a: i32, _b: i32, _loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as i32;
    let a = vm.pop() as i32;
...
fn tpl_or(vm: &mut CideVM, _a: i32, _b: i32, _loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as i32;
    let a = vm.pop() as i32;
    vm.push(if a != 0 || b != 0 { 1 } else { 0 });
    None
...
fn tpl_not(vm: &mut CideVM, _a: i32, _b: i32, _loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let a = vm.pop() as i32;
...
fn tpl_bit_or(vm: &mut CideVM, _a: i32, _b: i32, _loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as i32;
    let a = vm.pop() as i32;
    vm.push((a | b) as u64);
    None
...
fn tpl_shl(vm: &mut CideVM, _a: i32, _b: i32, loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as i32;
    let a = vm.pop() as i32;
    if !(0..32).contains(&b) {
...
fn tpl_shr(vm: &mut CideVM, _a: i32, _b: i32, loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as i32;
    let a = vm.pop() as i32;
    if !(0..32).contains(&b) {
...
fn tpl_ult(vm: &mut CideVM, _a: i32, _b: i32, _loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as u32;
    let a = vm.pop() as u32;
    vm.push(if a < b { 1 } else { 0 });
    None
...
fn tpl_ule(vm: &mut CideVM, _a: i32, _b: i32, _loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as u32;
    let a = vm.pop() as u32;
    vm.push(if a <= b { 1 } else { 0 });
    None
...
fn tpl_ugt(vm: &mut CideVM, _a: i32, _b: i32, _loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as u32;
    let a = vm.pop() as u32;
    vm.push(if a > b { 1 } else { 0 });
    None
...
fn tpl_uge(vm: &mut CideVM, _a: i32, _b: i32, _loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as u32;
    let a = vm.pop() as u32;
    vm.push(if a >= b { 1 } else { 0 });
    None
...
fn tpl_udiv(vm: &mut CideVM, _a: i32, _b: i32, loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as u32;
    let a = vm.pop() as u32;
    if b == 0 {
        vm.trap("除零错误：整数除法的除数不能为 0。", loc);
...
fn tpl_umod(vm: &mut CideVM, _a: i32, _b: i32, loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as u32;
    let a = vm.pop() as u32;
    if b == 0 {
        vm.trap("取模错误：取模运算的除数不能为 0。", loc);
...
fn tpl_lshr(vm: &mut CideVM, _a: i32, _b: i32, loc: &SourceLoc, _session: &mut VmContext<'_>) -> Option<StepResult> {
    let b = vm.pop() as i32;
    let a = vm.pop() as u32;
    if !(0..32).contains(&b) {
        vm.trap("移位操作越界：右移位数必须在 0~31 之间。", loc);
...
pub fn execute_trace_once(vm: &mut CideVM, session: &mut VmContext<'_>, trace: &CompiledTrace) -> Option<StepResult> {
