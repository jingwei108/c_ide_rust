...
- **覆盖**：298 个 Baseline 用例 + 82 个模板生成用例 + 76 个 K&R 用例 + 92 个 LeetCode 题（C Shadow Verification 合计 562 个用例，556 匹配）；83 个 C++ 用例（C++ Shadow Verification，83/83 全绿；2026-06-18 实测）
...
- **LeetCode**：已全面实施阶段 4 + 阶段 5，当前 92 道题全部通过，详见 `native/tests/LEETCODE_FAILURES.md`
...
- **函数返回 `double` 值异常** — 函数返回类型为 `double` 时，调用方通过返回值获取的结果可能为 `0.0`（在 `lc_4` 原始实现中发现：`double findMedianSortedArrays(...)` 在 Clang 下正确返回中位数，在 Cide VM 下输出全为 `0.00000`）。直接声明 `double x = 2.5;` 再打印正常，说明问题集中在函数返回路径而非 `double` 类型本身。workaround：需要返回浮点结果时改用整数缩放返回值，或在调用方通过指针输出。
- **`scanf` 的 `%s` 格式符暂不支持** — `scanf("%s", buf)` 无法正确读取字符串，运行后字符串内容为空。`%d`、`%c` 等格式符可正常工作。workaround：读取字符串时使用 `sscanf` 从已知字符串缓冲区解析，或改用 `getchar`/`fgets` 自行读取。
- **`fputs(str, stdout)` 无输出** — `fputs` 写入 `FILE*` 文件流正常，但写入 `stdout` 时不会产生任何输出。`printf`、`puts` 到 `stdout` 正常。workaround：向标准输出打印时改用 `printf` 或 `puts`。
- **`fclose` 后 VFS `FILE*` 仍被报告为内存泄漏** — 用户代码中正确调用 `fopen`/`fclose` 后，Cide 的运行时泄漏检测报告仍可能将 `FILE*` 标记为未释放。这是 VFS 实现中 `fclose` 未完全释放内部 FILE 结构体的表现，不影响程序 stdout，但会污染泄漏报告。workaround：教学演示中可忽略该误报，或在统一模式可视化中通过泄漏报告讲解"资源释放的重要性"时加以说明。
