...
- **覆盖**：305 个 Baseline 用例 + 82 个模板生成用例 + 81 个 K&R 用例 + 138 个 LeetCode 题 + 14 个 gap 用例（C Shadow Verification 合计 620 个用例，616 匹配；统计口径含 match + cide_better + known_issue）；93 个 C++ 用例（C++ Shadow Verification，93/93 全绿；2026-06-26 实测）
...
**C++ 子集边界（诚实记录）**：类类型作为 `vector<T>` / `list<T>` 等内置容器模板实参、非类型模板参数、嵌套类 `Outer::Inner` 实例化、const 引用参数、默认参数、自定义拷贝构造函数等特性暂不支持（2026-06-26 记录）。这些限制在 Cide C++ 教学子集当前 Stage 0~6 范围内尚未覆盖，后续按教学需求逐步扩展。
