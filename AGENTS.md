...
│  └─ K&R 验证"真实世界代码能不能跑"；LeetCode 因优先级调整暂时搁置 │
├────────────────────────────────────────────────────────────────┤
...
- **覆盖**：298 个 Baseline 用例 + 82 个模板生成用例 + 69 个 K&R 用例（C Shadow Verification 合计 463 个用例，450 匹配）；83 个 C++ 用例（C++ Shadow Verification，83/83 全绿）
...
- **LeetCode**：计划中，因 C++ 子集扩展优先级更高而暂时搁置，详见 `native/tests/LEETCODE_FAILURES.md`
...
- **VFS 文本模式换行转换** — Cide 虚拟文件系统不区分文本模式（`"r"`/`"w"`）与二进制模式（`"rb"`/`"wb"`），不会在 Windows 下将 `\n` 自动转换为 `\r\n`。这导致与 Windows 上 Clang 的文本模式 I/O（如 `vfs_io_extensions.c`、`file_fread.c`）存在stdout/ftell 差异。该差异属于已知的跨平台行为差异，当前不打算在 VFS 中模拟 Windows CRT 的换行转换，以简化教学 VM 实现；相关用例在 Shadow Verification 中保留为输出差异，作为诚实记录。
