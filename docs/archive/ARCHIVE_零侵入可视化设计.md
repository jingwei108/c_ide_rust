...
> **已归档（2026-09-13）**：检测器（`data_structure_detector.rs`）从未在后端实施，渲染层已随 2026-09-11 前端切割迁出；字段口径由 `docs/spec/STEP_PAYLOAD_SCHEMA_V0_1.md` 承载，能力缺口记录见 `docs/current/项目路线图.md` G9。其 §4 类型拓扑/字段偏移算法仍可作为未来落地链表/树检测器的详细设计参考。
