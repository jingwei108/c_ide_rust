...
- [x] **指针可视化**：`PointerVisTab` 已实现，显示指针变量列表和指向关系
- [x] **链表图可视化**：`LinkedListVisualizer` + `LinkedListVisTab` 已实现，支持时间旅行和渐进入场动画
- [x] **算法验证**：`AlgorithmTab` 已支持验证按钮和通过/失败结果展示
...
- [x] **更丰富的代码模板**：新增选择排序、插入排序、归并排序、链表操作、二叉树遍历、栈实现等模板
