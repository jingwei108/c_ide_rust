...
- 若无现成编号，可临时使用 `#DXX` 并在 `工程债务维护方案.md` 中追加。
- **前端相关债务编号（`D02`/`D06`/`D09`）已随前端切割（2026-09-11）冻结，不再新增**：这三项对应的 Dart/Flutter 代码已迁出本仓库（标签 `before-frontend-split`），编号保留在 `工程债务维护方案.md` 中备查，不得复用于新的 Rust 侧问题。
