...
> 构建状态：`cargo clippy` 0 警告，`cargo test` 200/200 全部通过（8 个测试文件：bytecode_gen 10 + compile_pipeline 13 + end_to_end_extra 117 + end_to_end 19 + lexer 10 + parser 12 + type_checker 12 + vm_memory_safety 7）
...
### 🟡 2.4 `host_realloc` 不支持就地缩容（in-place shrink）
...
### 🟡 2.5 `call_user_function` 的 `return_ip` 语义错误
...
### 🟡 2.7 `session.rs` 的 `#![forbid(unsafe_code)]` 产生假安全感
...
### 🟢 2.10 `OpCode::from_u8` 与 `#[repr(u8)]` 枚举值不一致
...
### 🟢 2.11 `host_scanf_n` 中 `%c` 跳过空白，与标准 C 语义不同
...
### 🟢 2.12 `compute_stride` 未处理 `dims[i] == 0` 的情况
...
### 🟡 2.14 Flutter: `toggleBreakpoint()` 断点操作低效
...
### 🟢 2.15 Flutter: `flutter_riverpod` 使用 dev 预发布版
...
### 🟡 3.2 `Expr::loc()` / `Expr::ty()` / `Expr::set_ty()` 大量重复 match
...
### 🟢 3.7 VM `step()` 中 `self.code[self.ip]` 每次指令都做数组边界检查
...
### 🟢 3.10 `capi/mod.rs` 中 `format_type` 与 `ast::Type::format_string` 功能重复
...
### 🟢 3.11 `host_memset` 使用 `store_i8` 逐字节写入
...
### 🟢 3.12 TypeChecker `visit_call` 内建函数可用 HashMap 替代 match 链
...
| `unsafe` 代码 | ✅ | 仅在 FFI 边界使用，核心逻辑 `forbid(unsafe_code)` |
...
### 🔴 紧急（影响正确性）
1. **修复 `printf` / `fprintf` 格式修饰符导致栈不平衡** — `host_funcs.rs:230-283` 及 `506-560`，最高优先级；建议提取公共格式解析函数
2. **修复逗号分隔多变量数组声明中维度丢失** — `parser.rs` 全局变量 + 局部变量两处
3. **修复 `unsigned char` 类型映射错误** — `parser.rs:366`
4. **修复 Flutter `reset()` 缺少 `await`** — `ide_notifier.dart:158`

### 🟡 重要（影响性能 / 长期稳定性）
5. **修复 `cide_get_runtime_error` 悬垂指针** — 改用 buffer 模式
6. **优化 Lexer `peek()` 性能** — 字符迭代改为字节索引，最大编译加速点
7. **重构 `Expr` match 宏** — 消除 `ast.rs` 约 100 行重复代码
8. **提取 `merge_free_list` 公共函数** — 消除 `host_funcs.rs` 约 40 行重复
9. **提取 `push_compile_messages` 统一函数** — 消除 `compile_pipeline.rs` 约 100 行重复
10. **实现 Parser 错误恢复** — 单一错误不再阻止后续代码检查

### 🟢 建议（提升工程质量）
11. TypeChecker `visit_call` 内建函数用 HashMap 替代 match 链
12. 添加 `proptest` 模糊测试 + `insta` 快照测试（Rust）
13. 为 `host_funcs` 每个宿主函数编写独立单元测试
14. 消除 Rust FRB API 层重复数据结构（`flutter_bridge.rs` ↔ `api/cide.rs`）
15. 拆分 `Session` 为 `CompileArtifact` + `RuntimeContext` + `VMOwner`
16. 引入 AST visitor trait 消除手写递归遍历
17. 优化 `toggleBreakpoint()` 为批量 API
18. 升级 `flutter_riverpod` 到稳定版
19. 拆分 `IdeState` 为子状态
20. 考虑 LSP 协议集成
