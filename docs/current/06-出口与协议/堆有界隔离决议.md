...
> 归属：主计划 [`后端定位与白箱计划.md`](../../01-定位与路线 的引擎层决策，Phase 1 内可实施（与切割无依赖）
>
...
1. **隔离窗口外的 UAF 可能漏检**：`free(p)` 与错误使用之间若隔离区已整体轮换（其间 churn 超过 256KB），`*p` 的访问落在已复用块上，仅表现为"读到别人的值"而非 UAF 诊断。与永久隔离的真实取舍，与 ASAN 行为一致，写入 `C语言子集规范.md` 已知差异："Cide 堆采用有界隔离（256KB）以强化 UAF 检测，隔离窗口内地址不复用（同 ASAN quarantine）"；
