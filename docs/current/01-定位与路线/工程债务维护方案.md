...
| 2026-09-11 | 4.42 | 复核外部 PR 清单（12 项）并启动分批修复：新增 [`ARCHIVE_代码审查复核20260911.md`](../../archive/ARCHIVE_代码审查复核20260911.md 记录独立复现结论（**12 项全部成立**，其中 3 处表述修正：P0-2 的 24 次越界描述落在"交换"而非"比较"模板、P1-6 实为 `W3053` 警告而 CLI 误标 `(E3053)`、P2-7c 字段名为 `ty_name`；另发现清单未提的 2 项同源问题：`code_line` 是跨文件全局偏移行号、同名变量会连带污染 `semantic_label`）。**批次 A 完成**：① P0-1 冒泡趟数文案 `n - i` → `i + 1`（`cide_algorithm_steps/src/sorting.rs`，实测 desc 由"第 1 趟：第 5 大"变为"第 1 趟：第 1 大"）；② P1-5 `cide_cli compile` 编译失败改为 `exit(1)`（实测坏文件 0 → 1，正常文件仍 0）；③ 引擎附注粘连：`RuntimeState::push_note` 统一补尾随 `\n`（仅 note 通道，stdout/stderr 逐字节保真），泄漏报告恢复逐行输出。回归：`cargo test`（end_to_end_extra / fuzz / host_contract / capi_first_batch / step_payload_schema / crash_regression）**366 passed / 0 failed**；`cargo build --release` 通过。批次 B~E（P2-7、P0-2/P0-3、P0-4、P1-6）待续 | 维护 Agent |
...
> 本方案应与 `AGENTS.md`、`CHANGELOG.md`、[`代码审阅与修复追踪20260906.md`](../../07-质量与裁定（修复进度的权威追踪）共同维护，任何新发现的工程债务应及时追加到本方案中（编号延续 `#DXX`，不重排）。
