...
> 当前对应的可观测手段：`cide_cli serve` 的 JSON-lines 会话帧、`cide_cli unified` 的执行摘要、capi 的 `cide_get_jit_stats` 等。⚠️ **尚无内存/堆统计的 capi 导出**——`cide_get_heap_stats_json` 目前只是 `后端定位与白箱计划.md` Phase 2b 第二批的计划项（**未实现**）；引擎内部已有 `build_heap_stats`（`native/crates/cide_runtime/src/memory_state.rs`）可作实现基础。
