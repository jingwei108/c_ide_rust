...
| 功能 | 判断 | 理由 |
|------|------|------|
| **F-01 智能检查点** | **必须** | 时间旅行调试核心体验。固定间隔策略在循环中浪费空间（循环 10000 次=500 个快照=500MB），智能模式可将内存降至 ~10MB。对移动端关键。 |完成
| **F-02 增量快照** | **必须** | 同上。50 个 1MB 全量快照 = 50MB，移动端不可接受。增量快照可降至 ~5MB。 |完成
...
## 修复状态记录（2026-06-05 当日）

| 编号 | 问题 | 状态 | 修复内容 |
|------|------|------|----------|
| BUG-001 | `write_cstring` 边界检查注释 | ✅ 已修复（历史版本） | 注释已明确说明 `<` 条件已为 `\0` 预留 1 字节空间 |
| BUG-002 | `MemoryState::default()` `heap_offset = 0` | ✅ 已修复（历史版本） | 手动实现 `Default`，`heap_offset` 已设为 `HEAP_START` |
| BUG-003 | `gen_addr` 未声明变量生成 `PushConst(0)` | ✅ **本次修复** | 改为提前 `return`，避免生成无意义指令；编译管线 `has_errors()` 守卫会拦截错误字节码 |
| BUG-004 | `session_ops.rs` 硬编码 `0x5000` | ✅ 已修复（历史版本） | 已改为引用 `crate::vm::vm::HEAP_START` |
| OPT-014 | `host_func_id.rs` 文件 I/O 注释过时 | ✅ 已修复（历史版本） | 已更新为"已在 host_funcs.rs / vfs.rs 完整实现"，移除 `#[allow(dead_code)]` |
| 文档-1 | `ROADMAP.md` "~30 条指令" | ✅ 已修复（历史版本） | 已更新为 "106 条指令" |
| 文档-2 | `AGENTS.md` "240 个单元测试" | ✅ 已修复（历史版本） | 已更新为 "331 个测试" |

OPT-007 删除 host_printf_0/1/2 死代码 ✅
OPT-013 生产代码 unwrap() 修复（7处）✅
OPT-012 format_string → Display 直接实现 ✅

**验证结果**：`cargo check` 通过，`cargo test` 331/331 全部通过。
