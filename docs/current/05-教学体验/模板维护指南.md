...
>
> **最后核对日期**：2026-09-13（修订：生成器 `scripts/sync_templates.py` 已由重构批次 R4 G1 于 2026-09-12 从历史提交恢复为**纯后端形态**——渲染模板源 + 生成 Clang Golden，不再产出任何 Flutter 资产；本文原"生成器已消失、用例静态留存"缺口叙述相应收敛。静态一致性校验 `test_templates.py` 仍未恢复，见 §5.1）
>
...
`source.c` 与 `source.cpp` 只能存在一个。`scripts/sync_templates.py`（R4 G1 恢复版）会扫描模板目录并按扩展名渲染；历史版本的 `index.json` 产物已随 2026-09-11 前端切割移除、不再生成。维护者仍应人工遵守该约定（见 §4）。
...
templates/<key>/source.c                     模板源（合法 C，人类维护）
        ↓  由 scripts/sync_templates.py 用默认参数渲染（R4 G1 恢复版，纯后端形态；见 4.2）
native/tests/cases_template_generated/<key>_default.c     82 个 .c（可重新生成）
        ↓
...
### 4.2 生成器现状：sync_templates.py 已恢复（R4 G1），静态一致性校验仍缺

- **已恢复（2026-09-12，重构批次 R4 G1，提交 `3c767bb`）**：`scripts/sync_templates.py` 自历史提交恢复为纯后端形态，承担三件事：扫描 `templates/` 读取 `source.c` + `meta.yaml`；渲染 `/*__PARAM_x__*/` → `cases_template_generated/*.c`；用 Clang 编译运行生成 `cases_golden/*.out`。原第 4/5 步（生成 `CideFlutter/assets/templates/index.json`、拷贝模板进 Flutter assets）已随前端切割**永久移除**。改模板后运行 `python scripts/sync_templates.py` 即可重新生成用例与 Golden。
- **仍未恢复**：静态一致性校验脚本 `scripts/test_templates.py` 不在仓库中（历史资产，已迁出），模板目录完整性核对仍依赖人工（见 §5.1）。
- 读取 `native/tests/cases_template_generated/` 的三处消费者不变：`native/tests/cide_e2e.rs`、`native/tests/shadow_verification/shadow_verify.py`、`scripts/extract_shadow_cases.py`。
...
- `native/tests/cases_template_generated/<key>_default.c` 与模板源保持同步（改模板后运行 `python scripts/sync_templates.py` 自动重新生成；一致性人工抽查）。
...
> 模板维护流程：改模板后运行 `python scripts/sync_templates.py` 重新生成用例与 Golden（R4 G1 恢复版）；静态一致性人工核对见 §5.1。
