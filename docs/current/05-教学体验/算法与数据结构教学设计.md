...
| `cide_cli serve` | JSON-lines（NDJSON）会话：`compile` / `run` / `step.*` / `payload.get`，见 [`CLI使用手册.md`](../../02-构建与上手 §6 |
...
> - 缺口 1（运行时算法属性验证，即项目路线图 G9）的重建排期锚定在 C# 主线 CS5（见 [`CSharp前端引入计划.md`](../../03-语言子集）；缺口 2（学习进度追踪）维持"进度状态由消费方持有"口径。
