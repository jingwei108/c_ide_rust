...
- [x] 234 个用例覆盖 baseline / double / function_pointer / file_io / union / long_long 等类别
...
| 模块 | 状态 | 测试 |
|------|------|------|
| 影子验证框架 | ✅ | 234 用例，216/216 baseline 匹配，3/3 double 匹配，2 个已知问题 |
...
| 特性 | 用例数 | 优先级 | 备注 |
|------|--------|--------|------|
| `function_pointer` 完整支持 | 3 | P1 | 当前仅支持函数名作为参数传递（`qsort`），不支持声明/调用语法 |
| `file_io`（`fopen`/`fread`/`fwrite`） | 3 | P1 | 需新增宿主函数 |
...
## 📊 影子验证基线数据（2026-05-17，扩展至 234 用例后）

```
总用例数: 234
完全匹配: 219 (93%) ← 新增 114 个 baseline 用例 + 修复短路求值 bug
编译缺口: 13 (5%)
运行时缺口: 0
输出差异: 0
已知问题: 2
...
**baseline 类别**：216/216 全部匹配 ✅  
**double 类别**：3/3 全部匹配 ✅（含 64 位精度验证）  
**已知问题**：`loop_var_shadow`（循环变量作用域）、`string_len_manual`（字符串存储长度）  
**已知缺失特性**：13 个编译缺口均为 Cide 明确不支持的语法

---

## 🎯 下一个 Sprint 推荐：C 子集扩展（数据驱动）

基于影子验证的编译缺口频率排序，Top 3 缺失特性：

| 特性 | 用例数 | 优先级 | 实现成本 | 备注 |
|------|--------|--------|----------|------|
| `function_pointer` 完整支持 | 3 | P1 | 高 | Parser 语法复杂，但 ROADMAP 明确 |
| `file_io`（`fopen`/`fread`/`fwrite`） | 3 | P1 | 中 | 新增宿主函数，教学场景高频 |
| `goto` | 1 | P2 | 中 | 控制流跳转，需 BytecodeGen 新增标签/跳转指令 |
...
### Sprint 2：`long long` 快速胜利 ✅ 已完成
> 分支：`feat/long-long-support`  
> 实际完成：2026-05-17  

- [x] Lexer：`long long` 关键字
- [x] AST：`TypeKind::LongLong`
- [x] Parser：`long long` / `long` 类型解析
- [x] TypeChecker：隐式转换链 `int → long long`
- [x] BytecodeGen：`PushConstI64` / `AddI64` / `CastI2I64` / ...
- [x] VM：新增 I64 运算指令
- [x] 端到端测试
...
| 优先级 | 特性 | 用例数 | 实现成本 | 备注 |
|--------|------|--------|----------|------|
| P1 | `function_pointer` 完整支持 | 3 | 高 | Parser 语法复杂，但 ROADMAP 明确 |
| P1 | `file_io`（`fopen`/`fread`/`fwrite`） | 3 | 中 | 新增宿主函数，教学场景高频 |
