...
| 2026-06-26 | 4.32 | 继续推进任务 G：新增 LeetCode/K&R/C++ E2E 用例 all in。LeetCode 新增 17 题（lc_7 / lc_16 / lc_17 / lc_22 / lc_24 / lc_29 / lc_38 / lc_40 / lc_50 / lc_54 / lc_168 / lc_242 / lc_344 / lc_412 / lc_509 / lc_704 / lc_746 / lc_977），扩展至 109 题；K&R 新增 5 个变体（kr_1_hello / kr_2_celsius / kr_4_atoi / kr_5_itoa / kr_6_getword），扩展至 81 个用例；C++ E2E 新增 10 题（cpp_pair_template / cpp_template_func_multi / cpp_reference_member / cpp_template_array / cpp_template_stack / cpp_unique_ptr_reset / cpp_class_array / cpp_ctor_init_list / cpp_reference_param_chain / cpp_function_overload_template），扩展至 71 题；探索用例 `cpp_move_semantics` 因触发内存泄漏报告未纳入。诚实记录 Cide C++ 子集不支持类类型模板实参、非类型模板参数、嵌套类实例化、const 引用参数、默认参数、自定义拷贝构造等特性。同步刷新 `engineering_health.md` 与 8.1/8.3 评估摘要；C Shadow Verification 606/610，C++ Shadow Verification 93/93；`cargo test --workspace --all-features` / `cargo clippy --workspace --all-targets --all-features -- -D warnings` / `cargo fmt --check` / `flutter analyze` / `flutter test` 全绿 | 维护 Agent |
| 2026-06-26 | 4.33 | 继续推进任务 G：第二批 all in 新增 20 道 LeetCode 简单/中等题（lc_7 / lc_67 / lc_83 / lc_190 / lc_191 / lc_202 / lc_205 / lc_219 / lc_231 / lc_263 / lc_292 / lc_345 / lc_349 / lc_367 / lc_383 / lc_389 / lc_392 / lc_401 / lc_409 / lc_415），LeetCode 防线扩展至 128 题；逐题生成 Clang golden 并验证 Cide Shadow Verification 无回归；修复 `lc_219` 实现兼容问题与 `lc_349` 输出格式问题；`lc_367` golden 生成时 Clang 链接报 `permission denied`，定位后重新生成成功。同步刷新 `engineering_health.md` 与 8.1/8.3 评估摘要；C Shadow Verification 606/610，C++ Shadow Verification 93/93；`cargo test --workspace --all-features` / `cargo clippy --workspace --all-targets --all-features -- -D warnings` / `cargo fmt --check` / `flutter analyze` / `flutter test` 全绿 | 维护 Agent |
| 2026-06-26 | 4.34 | 继续推进任务 G：第三批 all in 新增 10 道 LeetCode 简单/中等题（lc_257 / lc_290 / lc_326 / lc_404 / lc_448 / lc_496 / lc_617 / lc_36 / lc_150 / lc_199），LeetCode 防线扩展至 138 题；逐题生成 Clang golden 并验证 Cide Shadow Verification 无回归。诚实记录并绕过 3 处 Cide 与 Clang 行为差异/已知限制：① `(*int_ptr)++` 在 Cide 中实际按指针步长（+4）递增而非值 +1，需改用 `*p = *p + 1`；② `sprintf(fmt, ..., struct_ptr->member)` 变参传递时 Cide 会错误传递结构体指针地址而非成员值，需先提取到局部 int 再传入；③ 多维数组作为函数参数时 Cide 的行偏移计算错误，需改用一维数组手动索引。`lc_617` 中 `NULL` 作为三目运算符分支需显式转换为对应指针类型 `(struct TreeNode*)0` 以避免 Cide 报类型不匹配。同步刷新 `engineering_health.md` 与 8.1/8.3 评估摘要；C Shadow Verification 616/620，C++ Shadow Verification 93/93；`cargo test --workspace --all-features` / `cargo clippy --workspace --all-targets --all-features -- -D warnings` / `cargo fmt --check` / C++ Shadow Verification 全绿 | 维护 Agent |
| 2026-06-26 | 4.35 | 继续推进任务 B：拆分 `crates/cide_lexer/src/lib.rs`（1022 → 403 行），新增 `lexer/macros.rs`（内置预定义宏）与 `lexer/expand.rs`（对象式/参数化宏展开），解决 H01 参数化宏 `do { ... } while(0)` 包装逻辑与内置宏表膨胀导致的入口文件 oversized 问题；`cargo test --workspace --all-features` / `cargo clippy --workspace --all-targets --all-features -- -D warnings` / `cargo fmt --check` / C Shadow Verification 616/620 / C++ Shadow Verification 93/93 全绿 | 维护 Agent |
| 2026-06-26 | 4.36 | 推进 Phase 42：`cide_vec<T>` 类类型模板实参支持。`cide_typeck::cpp_monomorph.rs` 新增 `try_synthesize_builtin_container_class` / `synthesize_vec_class`，对类类型实参合成走普通类模板路径的 `cide_vec<T>` 实例化（含默认构造、析构、`size` / `get` / `push_back`）；`cide_codegen::expr::new_delete.rs` 修复 `delete[]` 无论元素是否有显式析构函数都必须释放 `base` 地址的问题；`cide_parser/src/cpp.rs` 透传类类型模板实参；新增 C++ E2E 用例 `cpp_cide_vec_class`（`// category: gap`，Clang++ 无法直接编译 Cide 内置容器）及其 golden；`scripts/shadow_verify_cpp.py` 支持首行 `// category:` 标记；`native/tests/CPP_FAILURES.md` 更新为 72 个 E2E 用例；`cide_list<T>` 类类型实参仍标记 `TODO` 待后续迭代。C Shadow Verification 616/620，C++ Shadow Verification 93/94；`cargo test --workspace --all-features`（752 passed） / `cargo clippy --workspace --all-targets --all-features -- -D warnings` / `cargo fmt --check` / `flutter analyze` / `flutter test` 全绿 | 维护 Agent |
| 2026-06-26 | 4.37 | 推进 Phase 42：`cide_list<T>` 类类型模板实参支持。`cide_typeck::cpp_monomorph.rs` 新增 `synthesize_list_class`，合成辅助节点类 `cide_list_node<T>` 与主类 `cide_list<T>` 的默认构造、析构、`size` / `get` / `push_back` / `clear`；将 `cide_list_node<T>` 推入 `pending_class_instantiations` 使其进入 `program.classes`；`cide_codegen::lib.rs` 修复类大小拓扑计算，字段类型为类时必须等依赖类大小计算完成；新增 C++ E2E 用例 `cpp_cide_list_class`（`// category: gap`）及其 golden；`native/tests/CPP_FAILURES.md` 更新为 73 个 E2E 用例；去掉 1 处 `TODO`。C Shadow Verification 616/620，C++ Shadow Verification 93/95；`cargo test --workspace --all-features`（752 passed） / `cargo clippy --workspace --all-targets --all-features -- -D warnings` / `cargo fmt --check` / `flutter analyze` / `flutter test` 全绿 | 维护 Agent |
| 2026-06-26 | 4.38 | 推进任务 B：拆分 `crates/cide_typeck/src/cpp_monomorph.rs`（1443 → 18 行入口），新增 `cpp_monomorph/func.rs`（函数模板单态化）、`cpp_monomorph/class.rs`（类模板单态化）、`cpp_monomorph/replace.rs`（类型替换：`replace_template_type` / `replace_template_types_in_stmt` / `replace_template_types_in_expr` / `resolve_template_id`）、`cpp_monomorph/synth.rs`（合成 AST 小工具）、`cpp_monomorph/builtin.rs`（内置容器分发入口）、`cpp_monomorph/builtin_vec.rs`（`cide_vec<T>` 类类型实参合成）、`cpp_monomorph/builtin_list.rs`（`cide_list<T>` 类类型实参合成）；`pub(crate)` 可见性调整保证跨子模块 impl 调用；所有子文件 <800 行；C Shadow Verification 616/620、C++ Shadow Verification 93/95 无新增失败；`cargo test --workspace --all-features` / `cargo clippy --workspace --all-targets --all-features -- -D warnings` / `cargo fmt --check` 全绿 | 维护 Agent |
...
- 探索用例 `cpp_move_semantics` 因触发 Cide 内存泄漏报告（移动构造后源对象未正确标记 region 转移），未纳入 E2E，已删除
- **诚实记录的新发现**：Cide C++ 子集当前不支持类类型作为模板实参传入 `vector<T>` / `list<T>`（`push_back` 无法匹配以类为参数的构造函数），也不支持非类型模板参数、嵌套类 `Outer::Inner` 实例化、const 引用参数、默认参数、自定义拷贝构造等特性。这些用例未纳入 E2E/Shadow。（注：`cide_list<T>` 类类型模板实参与 `const T&` 参数绑定右值已于后续 4.40 支持）
