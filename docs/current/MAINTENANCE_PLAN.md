...
|------|--------|------|----------|------|--------|
| D01 | 文档数据口径不一致 | README.md / CHANGELOG.md / 审阅报告 | 已统一为 555/562（2026-06-18 实测） | 外部信任损耗 | P0 |
| D02 | FRB 生成文件仍被追踪 | `.gitignore` 已排除但文件可能仍被 git 追踪 | 174KB + 222KB 生成代码在版本库中 | 合并冲突、版本库体积 | P0 |
| D03 | `codegen/expr.rs` 过大 | `crates/cide_codegen/src/expr.rs`（2047 行，已拆分为 8 个子模块并随 `cide_codegen` crate 迁移） | 单函数超千行 | 维护与审查成本高 | P1 |
...
|------|--------|------|------|----------------|------|
| D01 | 文档数据口径不一致 | 一 | ✅ 已完成 | 文档维护者 | 2026-06-18 实测统一为 555/562 |
...
| 维度 | 当前状态 |
|------|----------|
| C Shadow Verification | 555/562（98.9%），剩余 2 个模板代码运行时缺口 |
| C++ Shadow Verification | 83/83（100%） |
| E2E 回归 | Baseline/K&R/LeetCode/C++ 全绿，Template 78/82（4 已知偏差） |
| 三层契约 | Host/Bytecode/Differential 全绿 |
| Fuzz | 5/5 通过 |
| Clippy | 0 warning |
| 生产代码 unwrap/expect | 17 处（全量 45 处，已区分生产/测试/生成代码；统计范围扩展至 `native/crates/`） |
| TODO/FIXME | Rust 12 处、Dart 6 处 |
| 活跃失败记录 | 14 条（口径已统一） |
| 任务 B 超大文件拆分 | ✅ 已完成：`cide_typeck/src/builtin.rs`、`CideFlutter/lib/widgets/custom_keyboard.dart` 已拆分 |
| 已知限制修复 | `fputs(str, stdout)` 无输出 ✅ 已修复；剩余 8 项待持续处理 |
| 任务 G 推进发现 | JIT 统计信息污染 stdout 已修复，新增 `cide_get_jit_stats` C API |

主要短板：**Workspace 拆分已进入平台期（`cide_shared`/`cide_ast`/`cide_runtime`/`cide_vm`/`cide_lexer`/`cide_parser`/`cide_cpp_frontend`/`cide_typeck`/`cide_codegen`/`cide_algorithm_steps` 已独立；`unified/checkpoint.rs` 的 `CheckpointManager` 已下沉到 `cide_vm::snapshot`；剩余 `unified/types.rs`/`stream.rs`/`root_cause.rs`、`engine/`、`api/` 因 FRB 孤儿规则与 `Session` 耦合暂保留在 `cide_native` 内部）、编译管线三阶段镜像耦合、多入口 API 重复包装、`cide_diagnostics` 因 FRB 孤儿规则暂无法独立成 crate；AGENTS.md 中仍有 8 项已知限制（含 4 项当前不支持、4 项行为差异）待按优先级逐项修复**。
...
4. 生产代码 `unwrap/expect` 已降至 17 处，`unwrap_used` 已提升为 `deny`；统计口径以 `engineering_health.py` 排除 `#[cfg(test)]` / `#[test]` / `frb_generated.rs` 后的结果为准。

**验收标准**：

- ✅ 生产代码 `unwrap/expect` 已收敛至 17 处；`engineering_health.md` 与维护计划数字已对齐。
...
| 编号 | 限制项 | 类型 | 优先级 | 状态 |
|------|--------|------|--------|------|
| H01 | 参数化宏调用后带分号无法解析 | 不支持 | P2 | ⏳ 待评估 |
| H02 | VLA 边界检查缺失 | 不支持 | P2 | ⏳ 待评估 |
| H03 | `#include` 非标准库路径 | 不支持 | P3 | ⏳ 待评估 |
| H04 | `va_list` / `va_start` / `va_arg` / `va_end` | 不支持 | P3 | ⏳ 待评估 |
| H05 | 全局 VLA | 不支持 | P3 | ⏳ 待评估 |
| H06 | 复合副作用数组索引触发 NULL 指针陷阱 | 行为差异 | P1 | ⏳ 待评估 |
| H07 | 函数返回 `double` 值异常 | 行为差异 | P1 | ⏳ 待评估 |
| H08 | `scanf` 的 `%s` 格式符不支持 | 行为差异 | P1 | ⏳ 待评估 |
| H09 | `fputs(str, stdout)` 无输出 | 行为差异 | P1 | ✅ 已修复 |
| H10 | `fclose` 后 VFS `FILE*` 仍报告内存泄漏 | 行为差异 | P2 | ⏳ 待评估 |
...
| 任务 C | 第 5~6 周 | ✅ unwrap/expect 收敛 | 生产路径 17 处、`unwrap_used` 已为 deny |
...
| 任务 H | 持续推进 | 🚧 已启动 | `fputs(str, stdout)` 无输出已修复；剩余 8 项已知限制按优先级逐项评估修复 |
