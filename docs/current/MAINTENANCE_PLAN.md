...
| 维度 | 当前状态 |
|------|----------|
| C Shadow Verification | 505/511（98.8%），剩余 2 个模板代码 UB 缺口 |
| C++ Shadow Verification | 83/83（100%） |
| E2E 回归 | Baseline/K&R/LeetCode/C++ 全绿，Template 78/82（4 已知偏差） |
| 三层契约 | Host/Bytecode/Differential 全绿 |
| Fuzz | 5/5 通过 |
| Clippy | 0 warning |
| 生产代码 unwrap/expect | 17 处（全量 43 处，已区分生产/测试/生成代码） |
| TODO/FIXME | Rust 16 处、Dart 9 处 |

主要短板：**单 crate 体量过大、`codegen/stmt.rs` 仍 884 行超过 800 行目标、编译管线三阶段镜像耦合、多入口 API 重复包装、失败记录口径待澄清、CI 存在 Windows-only 与 Flutter patch 脆弱点**。
...
1. 在 `native/Cargo.toml` 中建立 workspace，逐步迁移 crate。
2. 优先拆分 **AST + VM**，因为二者被其他模块依赖最多，边界最清晰。
3. 每迁移一个 crate，执行 `cargo test` 与 Shadow Verification 验证无回归。
4. 最后统一 `cide_api` crate，收敛 FRB / C API / CLI 多入口重复包装。
...
**风险**：Workspace 拆分会改变模块间可见性，需大量调整 `pub` 与 `use`；建议小步迁移，一次一个 crate。
...
4. 当生产代码 `unwrap/expect` 降至 20 处以下时，将 `Cargo.toml` 中 `unwrap_used` 从 `warn` 提升为 `deny`。

**验收标准**：

- 生产代码 `unwrap/expect` ≤ 20 处。
...
3. **Android job 增加基础测试**：
   - 构建完成后至少运行 `flutter test --no-test-randomize-ordering-seed`（仅 Dart 层）。
...
1. **Dart Visualizer 缓存落地**：
   - `array_visualizer.dart`、`tree_visualizer.dart`、`linked_list_visualizer.dart` 中 `TextPainter`/`Path`/`Gradient` 缓存。
   - 为动画组件添加 `RepaintBoundary`。
   - 优化 `shouldRepaint` 判定。
2. **统一模式差分编码落地**：
   - 完成 `StepPayloadDelta` 全字段增量更新。
   - 大对象（符号表、变量历史）启用分页或懒加载。
   - 在 Dart 端使用 isolate 反序列化大状态。
3. **性能基线**：
   - 10 万步排序可视化 ≥55fps。
   - 内存占用不随步数线性增长。
...
1. **LeetCode 中等题逐步填充**：从 48 个简单题扩展至 20~30 个中等题。
...
- 新增用例均通过 Shadow Verification 或明确记录为 `KNOWN_*`。
...
| 阶段 | 时间 | 里程碑 | 关键交付物 |
|------|------|--------|------------|
| 任务 A | 第 1~4 周 | Workspace 拆分完成 | `native/Cargo.toml` workspace、各 crate 边界清晰、CI 通过 |
| 任务 B | 第 3~5 周 | 超大文件拆分完成 | 核心文件均 <800 行、测试全绿 |
| 任务 C | 第 5~6 周 | unwrap/expect 收敛 | 生产路径 ≤20 处、`unwrap_used` 提升为 deny |
| 任务 D | 第 5 周 | 失败记录口径统一 | `engineering_health.md` 数字口径一致、文档修正 |
| 任务 E | 第 6~8 周 | CI 与构建加固 | 无需临时 patch、Android 增加测试、本地与 CI 对齐 |
| 任务 F | 第 7~8 周 | 性能收尾 | Visualizer 缓存、差分编码、性能基线达标 |
| 任务 G | 第 9 周起持续 | 教学场景扩展 | LeetCode 中等题、K&R 进阶、知识卡片 |
