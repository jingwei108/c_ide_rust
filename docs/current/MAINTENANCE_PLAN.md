...
| C++ Shadow Verification | 93/93（100%） |
| E2E 回归 | Baseline/K&R/LeetCode/C++ 全绿，Template 78/82（4 已知偏差）；LeetCode 扩展至 138 题，K&R 扩展至 81 题，C++ E2E 扩展至 71 题 |
...
- **诚实记录的新发现**：Cide C++ 子集当前不支持类类型作为模板实参传入 `vector<T>` / `list<T>`（`push_back` 无法匹配以类为参数的构造函数），也不支持非类型模板参数、嵌套类 `Outer::Inner` 实例化、const 引用参数、默认参数、自定义拷贝构造等特性。这些用例未纳入 E2E/Shadow。
