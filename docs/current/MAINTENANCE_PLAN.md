...
|------|--------|------|----------|------|--------|
| D01 | 文档数据口径不一致 | README.md / CHANGELOG.md / 审阅报告 | 已统一为 504/511（2026-06-16 实测） | 外部信任损耗 | P0 |
...
- **执行步骤**：
  1. 在 CI 中增加以下指标统计并生成报告：
     - 各 Rust 源文件行数 Top 20
     - TODO/FIXME/HACK 数量
     - `unwrap`/`expect` 使用数量
     - 失败记录文件中活跃问题数量
     - Shadow Verification 匹配率趋势
  2. 将报告上传到 `reports/engineering_health.md`。
...
|------|--------|------|------|----------------|------|
| D01 | 文档数据口径不一致 | 一 | ✅ 已完成 | 文档维护者 | 2026-06-16 实测统一为 504/511 |
| D02 | FRB 生成文件管理 | 一 | ✅ 已完成 | CI/构建维护者 | 提交 `ab39aaa` 已改为构建时生成；`.gitignore` 已配置 |
| D03 | `codegen/expr.rs` 过大 | 二 | 待处理 | 编译器维护者 | 拆分为子模块 |
| D04 | `parser/mod.rs` 过大 | 二 | 待处理 | 编译器维护者 | 按语法结构拆分 |
| D05 | `host_funcs.rs` 过大 | 二 | 待处理 | VM 维护者 | 按功能拆分 |
| D06 | `ide_screen.dart` 过大 | 二 | 待处理 | 前端维护者 | 拆分为独立 widget |
| D07 | 缺少项目级 clippy 配置 | 三 | 待处理 | Rust 维护者 | 逐步收敛 |
| D08 | TODO/FIXME 标记极少 | 三 | 待处理 | 全团队 | 制定规范 |
| D09 | CustomPainter 缺少缓存 | 三 | 待处理 | 前端维护者 | 性能优化 |
| D10 | C++ 扩展模块耦合度高 | 四 | 待处理 | C++ 扩展维护者 | 建立子模块边界 |
| D11 | 失败记录文件稀释 | 一 | ⏸️ 保留不动 | 测试维护者 | 按用户决策，暂不归档 |
| D12 | Mutex poison 静默恢复 | 五 | 待处理 | 桥接维护者 | 增加日志 |
