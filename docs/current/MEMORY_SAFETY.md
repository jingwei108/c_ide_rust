...
> **适用范围：** `native/`（纯后端 Rust 仓库：编译器/VM 核心 + 三出口 C ABI / wasm32 / `cide_cli serve`）。前端（Flutter/Dart）已于 2026-09-11 切割移交社区，本仓库不再包含 Dart 代码。
>
...
> | 契约 | 前端需保证 |
> |------|-----------|
> | 会话释放 | 每创建一个引擎会话（C ABI `cide_session_create` / serve 会话）必须成对销毁；应用退出时集中清理 |
> | 资源释放 | 控制器、监听器、动画对象在组件销毁时对称释放，禁止长期存活的无名监听器 |
> | 异步取消 | 运行中被替换/取消的任务必须真正停止，且不得继续持有会话：引擎侧可用手段为 `cide_set_max_steps`（步数上限）/ 会话销毁 / serve 的 `session.reset`（**当前没有 `cide_stop` 一类的异步中断 API**，如实记录） |
...
- **设计决议全文**：[`CIDE_HEAP_QUARANTINE_DECISION.md`](CIDE_HEAP_QUARANTINE_DECISION.md)（2026-09-11 拍板并实施；含验收清单 §6）
- **会话可配置**：隔离预算可按会话调参（capi `cide_set_quarantine_budget` / `cide_get_quarantine_budget`，默认 256KB）
...
> 当前对应的可观测手段：`cide_cli serve` 的 JSON-lines 会话帧、`cide_cli unified` 的执行摘要、capi 的 `cide_get_jit_stats` 等。⚠️ **尚无内存/堆统计的 capi 导出**——`cide_get_heap_stats_json` 目前只是 `CIDE_BACKEND_SPLIT_WASM_WHITEBOX_PLAN.md` Phase 2b 第二批的计划项（**未实现**）；引擎内部已有 `build_heap_stats`（`native/crates/cide_runtime/src/memory_state.rs`）可作实现基础。
...
| `current/CIDE_HEAP_QUARANTINE_DECISION.md` | 堆内存 Bump 分配 + 有界隔离决议（256KB 隔离预算） |
| `spec/STEP_PAYLOAD_SCHEMA_V0_1.md` | 三出口载荷 schema（指针四状态等） |
| `current/CIDE_BACKEND_SPLIT_WASM_WHITEBOX_PLAN.md` | 前端切割与三出口路线 |
