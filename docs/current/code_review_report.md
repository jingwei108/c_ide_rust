...
| # | 文件:行 | 状态 | 描述 |
|---|---------|------|------|
| D3 | compiler/ast.rs:912 | 真死码 | impl Default for Expr —— 从未使用 |
| D4 | compiler/codegen/mod.rs:46 | 真死码 | ClassVarEntry.name —— 写入但从不读取（仅 .class_name 和 .offset 被访问） |
| D5 | compiler/typeck/mod.rs:1003-1011 | 真死码 | pub(crate) fn get_class_field_type() —— 0 个调用点 |
| D6 | compiler/cpp_frontend/builtin_layout.rs:29,38,41 | 装饰死码 | source_file、version、generated_at —— JSON 反序列化后从未读取 |
| D7 | compiler/cpp_frontend/type_map.rs:8,11 | 装饰死码 | version、generated_at —— 同上 |
| D8 | compiler/cpp_frontend/type_map.rs:12-15 + builtin_layout.rs | 重复加载 | 同一 JSON 文件被两个模块各自 include_str! 加载一次 |
| D9 | lib/widgets/symbol_bar.dart (82行) | 真死码 | SymbolBar 类 —— 整个项目中零次导入/实例化（custom_keyboard 使用同名私有函数 _buildSymbolBar） |
| D10 | lib/widgets/symbol_chip.dart (33行) | 真死码 | SymbolChip 类 —— 仅被已死的 SymbolBar 使用 |
| D11 | lib/widgets/call_stack_tree.dart (111行) | 真死码 | CallStackTree 类 —— 从未实例化；CallstackTab 自行构建 UI |
| D12 | lib/widgets/concept_graph_view.dart:220 | 真死码 | const y = 12.0 —— 声明但从未使用（另一处 347 行同名变量在使用） |
| D13 | lib/providers/unified_notifier.dart:18-22 | 伪死码 | @override dispose() —— 不覆写任何方法，应移除 |
...
| S7 | compiler/cpp_frontend/type_map.rs:34,50 | 低 | Box::leak(cide_name.clone().into_boxed_str()) 每次调用都泄漏字符串。 | 使用 LazyLock<HashMap<&'static str, &'static str>> 一次性缓存 |
| S8 | compiler/cpp_frontend/builtin_layout.rs:138-139 | 低 | builtin_class_mappings() 每次调用都 Box::leak 新字符串。 | 同上，使用 LazyLock 缓存 |
...
**Flutter analyze lib test 结果（4 issues）**：
1. `providers/unified_notifier.dart:19` — override_on_non_overriding_member
2. `providers/unified_notifier.dart:21` — undefined_super_member
3. `widgets/concept_graph_view.dart:220` — unused_local_variable
4. `widgets/learning_path_panel.dart:221` — use_build_context_synchronously
...
1. 清理确认的死代码（D3-D13），但跳过 D1/D2 等误判项
2. 性能优化 O1-O15
3. 架构重构 A1-A14
4. 为每个 P0/P1 问题补充回归测试
5. 补齐 C API `# Safety` 文档
6. 扩展 Flutter 测试金字塔
7. 分析并修复 Shadow Verification 的 39 个缺口
