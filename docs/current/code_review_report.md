...
| Flutter 测试 | 薄弱 | 仍仅 1 个测试文件、2 个测试 |
...
- **当前测试状态**：691 passed / 0 failed，Clippy 0 warnings，Flutter analyze 0 issues。
...
说明：编译缺口减少 5 个，其中 `bellmanFord_default`、`spfa_default`、`threadedBinaryTree_default` 原属 `unknown`；`kr_5_10` 原因三目字符串字面量类型不匹配编译失败；`kr_5_16` 原因匿名 enum 不支持编译失败。这 5 个用例中仅 `bellmanFord_default` 实现完整匹配，其余 4 个进入运行阶段后因其他限制仍失败（如 `spfa_default` 因数组越界检查、`kr_5_10` 因缺少 `argc/argv` 支持），故运行时缺口增加 4。剩余 2 个 `unknown` 编译缺口为 `kr_5_8`、`kr_5_14`（复杂函数指针类型转换）。

### 13.4 剩余高价值缺口

1. **`kr_5_8` / `kr_5_14`**：复杂函数指针类型转换 `(int (*)(void*, void*))` parser 不支持，为最后 2 个 baseline 编译缺口。
2. **`argc/argv` 支持**：解锁 `kr_5_10` 及多个依赖命令行参数的 K&R 用例。
3. **Windows 文本换行转换**：`vfs_io_extensions`、`file_fread` 输出差异根因。
4. **大量 K&R 运行时缺口**：需逐个用例定位（如 `kr_1_*`、`kr_4_*`、`kr_5_*`、`kr_6_*` 系列）。
