...
> **定位扩展（2026-09-12 同步声明，CSHARP_EXTENSION_PLAN.md §11 待办①）**：
> 在 C/C++ 主轴之上新增 **C# 教学子集作为第二前端**（SharpTutor 锚定，启动时序
> 与批次见 [`CSHARP_EXTENSION_PLAN.md`](CSHARP_EXTENSION_PLAN.md)）。本文件的
...
│    第一消费者：cide_cli、shadow_verify.py（ctypes，632 用例生产验证）
...
│
└─ cide_cli serve（JSON-lines 会话模式，新增）
...
> 2026-09-11 修订：schema 定稿从 Phase 3 前置到 Phase 1——第一批 `cide_step_next_json` 的输出即 StepPayload，协议不可能晚于消费它的 API（SharpTutor 评审论据，已采纳）。CoW 等引擎内部优化仍以 schema v0.1 定稿为前置。
...
| 层级 | 范围 | 结果 |
|---|---|---|
| cargo check 层 1~3 | 全部 10 个编译器 crate + runtime + vm + cide_native 主 crate | ✅ 零错误 |
| release 构建 | `cide_native.wasm` **3.75MB**（`cide_cli.wasm` 4.46MB 同样产出） | ✅ |
...
- `cide_abi_version()` 引入 + capi 变更日志（语义化版本：加函数=minor，改签名/语义=major）；
- FRB 生成代码随前端走，本仓库删除 `flutter_rust_bridge` 依赖（这也是 wasm 门控的一部分）。

### 5.3 capi 补全分批（Issue C 落地方案，已按 SharpTutor 评审修订，见 `CIDE_CAPI_REVIEW_RESPONSE.md`）

横切契约（全部批次适用）：复杂返回 JSON 字符串 + **rust-alloc 所有权**（`cide_free_string` 释放，废弃 caller-buffer 双轨）；所有新入口 `catch_unwind`；状态码 `0=成功/负数=入参错误/正数=领域状态`（1=trap、2=waiting_input）；Session 指针句柄**非线程安全**（跨线程需调用方同步）；UTF-8 输入输出契约。
...
3. **前端真空期**：切割后到社区前端成熟前，本项目自身无 GUI——`cide_cli`（compile/run/step/unified）是过渡期唯一交互形态，其体验需要维护好；
4. ~~**SharpTutor 依赖协调**~~（✅ 已完成，2026-09-11）：capi 第一批签名经 SharpTutor 完整评审并达成一致，逐条结论与六个开放问题的回答见 [`CIDE_CAPI_REVIEW_RESPONSE.md`](CIDE_CAPI_REVIEW_RESPONSE.md)（含三项实证：重复编译内存有界、alloc_line 已存在、frameCache 越窗行为）；其 §8 对等承诺（15 用例冒烟集 / capi/serve 骨架 PR / schema 回放场景）附 review 基线已接受；
