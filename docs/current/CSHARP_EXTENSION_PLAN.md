...
> `CIDE_CAPI_REVIEW_RESPONSE.md`）。在此之上，其"多语言对比教学"要从静态并排面板升级为
...
>
> **执行时机**：本仓库 `CIDE_RESTRUCTURE_PLAN.md` 全批次交付后启动（时序见 §7）。
...
扩展，**待在 `CIDE_BACKEND_SPLIT_WASM_WHITEBOX_PLAN.md` 同步声明**（待办，见 §11）。
...
**整备计划时序同步（2026-09-12 v1.1 吸收）**：[`CIDE_TRILINGUAL_OVERHAUL_PLAN.md`](CIDE_TRILINGUAL_OVERHAUL_PLAN.md)
...
[`CIDE_OVERHAUL_ROADMAP.md`](CIDE_OVERHAUL_ROADMAP.md)（U0~U7），本计划的硬门禁以路线图
...
**中止与改判判据（2026-09-12 v1.2 吸收，[`CIDE_CORE_ASSET_RECONSTRUCTION_VERDICT.md`](CIDE_CORE_ASSET_RECONSTRUCTION_VERDICT.md) §7.3/§Q6）**：
...
- **三语化整备吸收（2026-09-12，整备计划 v1.1）**：[`CIDE_TRILINGUAL_OVERHAUL_PLAN.md`](CIDE_TRILINGUAL_OVERHAUL_PLAN.md)
